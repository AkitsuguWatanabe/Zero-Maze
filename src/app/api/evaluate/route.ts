...
import { extractStructured } from "@/lib/evaluate-core";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Extraction only — final_instruction/milestones are generated by a
    // follow-up call to /api/evaluate/finalize, made once the user confirms
    // via "この内容で指示文を作成する". This is what actually cut the 120s+
    // timeouts documented above: most calls here no longer pay for
    // generating a final_instruction upfront. Kept as a full Evaluation
    // shape (empty final_instruction, null milestones, business_category
    // filled in client-side from the confirm-step classification) so
    // existing clients/types don't need to change for the fields unaffected
    // by the split.
    const extraction = await extractStructured(draft, rank, mode, modelOverride, categories);
    return NextResponse.json({ ...extraction, business_category: null, final_instruction: "", milestones: null });
  } catch (err) {
    ...
    return NextResponse.json(
      { error: "確認中にエラーが発生しました。お手数ですが、もう一度「この内容を確認する」を押してください。" },
      { status: 500 },
    );
  }
}
