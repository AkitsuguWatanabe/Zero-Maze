...
export async function GET() {
  ...
  try {
    ...
    if (ctx.role === "reseller_admin") {
      // Look up this admin's reseller_id via user_roles
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const insertData: Record<string, unknown> = { name, reseller_id: resellerId, status: "active" };
    if (body.slug?.trim()) insertData.slug = body.slug.trim();
...
export async function PATCH(req: NextRequest) {
  ...
  // Advanced config (status, Google Sheets, AI model overrides) — super_admin only
  if (ctx.role === "super_admin") {
    ...
    if (body.googleSheetId !== undefined) updates.google_sheet_id = body.googleSheetId.trim() || null;
...
export async function DELETE(req: NextRequest) {
  const ctx = await getCurrentUserContext();
  // Only super_admin can delete tenants (irreversible, cascades to all their data)
